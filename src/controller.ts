...
@odata.type(Direccion)
export class DireccionesController extends ODataController {
    @odata.GET
    async find(@odata.query query: ODataQuery): Promise<Direccion[]> {
        const db = await connect();
        const mongodbQuery = createQuery(query);
        if (typeof mongodbQuery.query._id == "string") mongodbQuery.query._id = new ObjectID(mongodbQuery.query._id);
        let result = typeof mongodbQuery.limit == "number" && mongodbQuery.limit === 0 ? [] : await db.collection("Direcciones")
            .find(mongodbQuery.query)
            .project(mongodbQuery.projection)
            .skip(mongodbQuery.skip || 0)
            .limit(mongodbQuery.limit || 0)
            ...
            .toArray();
        if (mongodbQuery.inlinecount) {
            (<any>result).inlinecount = await db.collection("Direcciones")
                .find(mongodbQuery.query)
                .project(mongodbQuery.projection)
                .count(false);
        }
    ...
    @odata.GET
    async findOne(@odata.key key: string, @odata.query query: ODataQuery): Promise<Direccion> {
        const db = await connect();
        const mongodbQuery = createQuery(query);
        let keyId;
        try { keyId = new ObjectID(key); } catch (err) { keyId = key; }
        return db.collection("Direcciones").findOne({ _id: keyId }, {
            fields: mongodbQuery.projection
        });
    }

    @odata.POST
    async insert(@odata.body data: any): Promise<Direccion> {
        const db = await connect();
        return await db.collection("Direcciones").insertOne(data).then((result) => {
            data._id = result.insertedId;
            return data;
        });
    }
    ...
    @odata.PUT
    async upsert(@odata.key key: string, @odata.body data: any): Promise<Direccion> {
        const db = await connect();
        let keyId;
        try { keyId = new ObjectID(key); } catch (err) { keyId = key; }
        return await db.collection("Direcciones").updateOne({ _id: keyId }, data, {
            upsert: true
        }).then((result) => {
            data._id = result.upsertedId
            return data._id ? data : null;
    ...
    async update(@odata.key key: string, @odata.body delta: any): Promise<number> {
        ...
        let keyId;
        try { keyId = new ObjectID(key); } catch (err) { keyId = key; }
        return await db.collection("Direcciones").updateOne({ _id: keyId }, { $set: delta }).then(result => result.modifiedCount);
    }
